...
describe("Onboarding - Company address flow", () => {
  ...
  it("displays the company addresses page", () => {
    ...
    cy.get('[data-testid="subtitle"]').should("contain", "Where are you operating from?");
    cy.contains("button", "Continue").should("be.visible").and("be.enabled");
    cy.get('[data-testid="country-state"]').should("exist");
    cy.get('[data-testid="zipcode-state"]').should("exist");
    cy.get('[data-testid="province-state"]').should("exist");
    cy.get('[data-testid="city-state"]').should("exist");
  });

  it("back arrow goes back to company website page", () => {
    cy.get('[data-testid="country-state"]').click();
  ...
  it("displays and validates zipcode input", () => {
    cy.get('[data-testid="zipcode-state"]').click();
  ...
  it("displays and validates province input", () => {
    cy.get('[data-testid="province-state"]').click();
  ...
  it("displays and validates city and addresses input", () => {
    cy.get('[data-testid="city-state"]').click();
